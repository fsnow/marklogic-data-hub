  ...
  if (fn:exists($document)) then (
    ...
    let $minimum-threshold-combinations-queries :=
      ...
      let $queries :=
        ...
        let $queries := match-impl:query-map-to-query($document, $query-map, $values-by-property-name, $cached-queries, $query-prov, $target-entity)
      ...
      let $not-queries :=
        ...
        (: if there are no values from the document for a query, then match-impl:query-map-to-query returns an empty sequence  :)
        let $queries := match-impl:query-map-to-query($document, $not-query-map, $values-by-property-name, $cached-queries, (), $target-entity)
        return if (fn:count($queries) gt 1) then
        (: if a match ruleset has multiple queries, be sure to AND them :)
          helper-impl:group-queries-by-scope($queries, cts:and-query#1)
        else
          ...
          element results {
            ...
            if ($include-results and $estimate gt 0) then
              let $queries-for-scoring :=
                ...
                let $query := match-impl:query-map-to-query($document, $query-map, $values-by-property-name, $cached-queries, $query-prov, $target-entity)
                let $_trace :=
                  if (xdmp:trace-enabled($const:TRACE-MATCH-RESULTS)) then
    ...
    let $queries := (
      ...
      let $queries :=
        if ($sub-query-map => map:get("type") = "reduce") then
          ...
          (: let $log := xdmp:log("************ " || $property-name || " isMultiStructPropMultiValueComponent : " || map:get($sub-query-map, "isMultiStructPropMultiValueComponent")) :)
  ...
  as cts:query*
{
  ...
  return
    if (fn:empty($multi-struct-prop-multi-value-map) or fn:empty($target-entity)) then
      ...
      return $parent-query-fn($query, $weight)
};
