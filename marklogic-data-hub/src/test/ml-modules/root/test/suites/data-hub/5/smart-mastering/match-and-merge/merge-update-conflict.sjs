...
const assertions = [];
assertions.push(
  test.assertTrue(fn.empty(merge.main({uri: lib['MATCH-SUMMARY-URI-1']}, options)), 'Since there is another merge containing more documents this merge should result in empty sequence')
);

const results = merge.main({uri: lib['MATCH-SUMMARY-URI-2']}, options).toArray().map((contentObj) => contentObj.uri);
assertions.push(
